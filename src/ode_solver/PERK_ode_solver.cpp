...
//    this->rk_stage[istage]=0.0; //resets all entries to zero
    ...
    for (size_t k = 0; k < group_ID.size(); ++k){
        this->rk_stage_k[0][istage]=0.0;
        for (int j = 0; j < istage; ++j){
            if (this->butcher_tableau->get_a(istage,j, 1) != 0){
                this->rk_stage_k[0][istage].add(this->butcher_tableau->get_a(istage,j, 1), this->rk_stage_k[0][j]);
            }
        ...
        if(pseudotime) {
            const double CFL = dt;
            this->dg->time_scale_solution_update(this->rk_stage_k[0][istage], CFL);
        }else {
            this->rk_stage_k[0][istage]*=dt;
        }//dt * sum(a_ij * k_j)
        
        this->rk_stage_k[0][istage].add(1.0,this->solution_update); //u_n + dt * sum(a_ij * k_j)
        ...
        if (!this->butcher_tableau_aii_is_zero[istage]){
            ...
            */
            ...
            //JFNK version
            this->solver.solve(dt*this->butcher_tableau->get_a(istage,istage,1), this->rk_stage_k[0][istage]);
            this->rk_stage_k[0][istage] = this->solver.current_solution_estimate;
    ...
    // u_n + dt * sum(a_ij * k_j) <explicit> + dt * a_ii * u^(istage) <implicit>
        ...
        this->relaxation_runge_kutta->store_stage_solutions(istage, this->rk_stage_k[0][istage]);
...
void PERKODESolver<dim,real,n_rk_stages,MeshType>::sum_stages (real dt, const bool pseudotime)
{
 ...
 //           for (size_t k = 0; k<group_ID.size(); ++k){
                this->solution_update.add(dt* this->butcher_tableau->get_b(istage),this->rk_stage_k[0][istage]);
 //           }
        }
    }
